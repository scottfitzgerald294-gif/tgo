...
from app.adapters.pdd import MockPddAdapter, MockPddSendError
...
@pytest.mark.unit
def test_mock_adapter_fails_configured_attempts_and_deduplicates_reply_id() -> None:
...
@pytest.mark.unit
def test_mock_adapter_deduplicates_synthetic_inbound_message_key() -> None:
    adapter = MockPddAdapter()
    inbound = build_message()
    key = ConversationKey(
        shop_id=inbound.shop_id,
    ...
    assert adapter.inbound_count == 1
    assert transcript is not None
    assert [item.direction for item in transcript.messages] == ["buyer"]
